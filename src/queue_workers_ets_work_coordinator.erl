...
handle_call({run_job, Job}, From, #{ job_table := Tid,
                                     tblname := TblName,
                                     worker_index := WI } = State) ->
    true = ets:insert(Tid, {From, Job}),
    Worker = queue_workers_ets_sup:worker_name(TblName, WI),
    ok = gen_server:cast(Worker, {sync_new_job, TblName, From, Job}),
...
handle_info({sync_new_job_results, _TblName, From, JobResult}, State) ->
    gen_server:reply(From, JobResult),
    {noreply, State};
handle_info(new_job, #{ tblname := TblName,
                        worker_index := WI } = State) ->
